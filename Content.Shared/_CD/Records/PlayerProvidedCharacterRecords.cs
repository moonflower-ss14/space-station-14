...
using Content.Shared.Humanoid.Prototypes; // Starlight
...
public sealed partial class PlayerProvidedCharacterRecords
{
    ...
    // Starlight varient that accepts a species prototype. Some Moonflower variants. MIT license.
    public static PlayerProvidedCharacterRecords DefaultRecords(SpeciesPrototype species)
    {
        return new PlayerProvidedCharacterRecords(
            hasWorkAuthorization: true,
            height: (int)(species.StandardHeight * (species.DefaultHeight - 1f) * 2f + species.StandardHeight),
            weight: 70,
